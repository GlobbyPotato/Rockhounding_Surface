...
public class TopMachines implements IProbeInfoProvider{
	...
	public static class EnergyInfo implements IProbeConfigProvider{
		...
		public void getProbeConfig(IProbeConfig config, EntityPlayer player, World world, IBlockState blockState, IProbeHitData data) {
			...
			if(te instanceof TileEntityWoodIncubator || te instanceof TileEntityCompostBin
			){
				config.setRFMode(0);
			}			
		}
	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world, IBlockState blockState, IProbeHitData data) {
		...
		if(te != null){
			if(te instanceof TileEntityCompostBin){
				...
				probeInfo.progress(tank.getCompost(), tank.getCompostMax(), probeInfo.defaultProgressStyle()
																	.suffix(" unit/s")
