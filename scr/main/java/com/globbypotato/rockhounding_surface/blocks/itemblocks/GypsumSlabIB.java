...
public class GypsumSlabIB extends MetaIB {
        ...
        if (stack.stackSize != 0 && playerIn.canPlayerEdit(pos, facing, stack) && worldIn.canBlockBePlaced(this.block, pos, false, facing, (Entity)null, stack)){
            int i = this.getMetadata(stack.getMetadata());
	            if((facing.getIndex() > 1 && hitY > 0.5F) || (facing.getIndex() == 0)){
	            	placingState = ModBlocks.gypsumSlabsHi.getDefaultState().withProperty(GypsumSlabs.VARIANT, EnumGypsumBlocks.values()[stack.getItemDamage()]);
	            }else{
	               	placingState = ModBlocks.gypsumSlabsLo.getDefaultState().withProperty(GypsumSlabs.VARIANT, EnumGypsumBlocks.values()[stack.getItemDamage()]);
	            }
