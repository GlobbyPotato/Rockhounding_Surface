...
public class ColdLogs extends BaseLog {
    public static final PropertyEnum VARIANT = PropertyEnum.create("variant", EnumColdLogs.class);
    ...
    public IBlockState getStateFromMeta(int meta){
        IBlockState iblockstate = this.getDefaultState().withProperty(VARIANT, EnumColdLogs.values()[(meta & 3) % 4]);
        switch (meta & 12){
    ...
    public int getMetaFromState(IBlockState state){
        int i = 0;
        i = i | ((EnumColdLogs)state.getValue(VARIANT)).ordinal();
    ...
    public ItemStack createStackedBlock(IBlockState state){
        return new ItemStack(Item.getItemFromBlock(this), 1, ((EnumColdLogs)state.getValue(VARIANT)).ordinal());
    }

    @Override
    public int damageDropped(IBlockState state){
        return ((EnumColdLogs)state.getValue(VARIANT)).ordinal();
    }
}