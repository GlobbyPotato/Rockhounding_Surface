...
public class TileEntityMachineEnergy extends TileEntityMachineInv  implements IStorage {
	...
	public boolean canInduct() {
		return hasPermanentInduction() || (!hasPermanentInduction() && input.getStackInSlot(FUEL_SLOT) != null && input.getStackInSlot(FUEL_SLOT).isItemEqual(SupportUtils.inductor()));
	}
	...
	protected void fuelHandler(ItemStack stack) {
		if(stack != null) {
			...
			if(ItemStack.areItemsEqual(stack, SupportUtils.inductor())){
