    ...
    public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ){
        ...
        if (stack.getCount() != 0 && playerIn.canPlayerEdit(pos, facing, stack) && canBlockBePlaced(iblockstate, this.block, worldIn, pos, facing)){
            if((facing.getIndex() > 1 && hitY > 0.5F) || (facing.getIndex() == 0)){
            	this.placingState = ModBlocks.FOSSIL_SLABS_HI.getDefaultState().withProperty(FossilSlabs.VARIANT, EnumFossilPlanks.values()[stack.getItemDamage()]);
            }else{
               	this.placingState = ModBlocks.FOSSIL_SLABS_LO.getDefaultState().withProperty(FossilSlabs.VARIANT, EnumFossilPlanks.values()[stack.getItemDamage()]);
            }
